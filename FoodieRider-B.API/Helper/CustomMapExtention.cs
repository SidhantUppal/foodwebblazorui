...
namespace FoodieRider.API.Helper
{
    public static class CustomMapExtention
    {
        ...
        where TUser : class, new()
        {
            ...
            routeGroup.MapPost("/register", async Task<Results<Ok, ValidationProblem>>
                (UserDto registration, HttpContext context, [FromServices] IServiceProvider sp) =>
            {
                ...
                await SetFirstName(user as User, firstName, lastName, CancellationToken.None);
            ...
            accountGroup.MapGet("/user", async (ClaimsPrincipal claimsPrincipal, [FromServices] IServiceProvider sp) =>
            {
                var userManager = sp.GetRequiredService<UserManager<TUser>>();
                var userResponse = await userManager.GetUserAsync(claimsPrincipal) as User;
                if (userResponse is not { } user)
                {
        ...
        public static Task SetFirstName(User user, string firstName, string lastName, CancellationToken cancellationToken)
        {
